...
def test_get_token_returns_identies_api_key_from_settings():
    settings = SimpleNamespace(identies_api_key="env-token")
...
def test_get_token_ignores_blank_identies_api_key():
    settings = SimpleNamespace(identies_api_key="   ")
    provider = AuthTokenProvider(settings=settings)
...
def test_get_token_uses_os_env_fallback():
    settings = SimpleNamespace()
    provider = AuthTokenProvider(settings=settings)

    with (
        patch(
            "tessera_sdk.infra.auth_token_provider.os.getenv",
            return_value="os-env-token",
        ),
        patch("tessera_sdk.infra.auth_token_provider.get_m2m_token_sync") as m2m,
    ):
        token = provider.get_token()

    assert token == "os-env-token"
    m2m.assert_not_called()
